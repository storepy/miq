...
def get_ig_username_media(data: dict):
    if url := get_dict_key(data, 'graphql__user__profile_pic_url_hd'):
        img = download_img_from_url_to_b64(url, s=s, headers=headers, cookies=cookies)
        print(1)
        if img:
...
def map_ig_username_info_res(data: dict):
    ...
    # base_keys = [
    #     'id', 'biography',
    #     'full_name', 'is_business_account',
    #     'category_name',
    #     'business_address_json',
    #     'business_email', 'business_phone_number', 'business_category_name', 'connected_fb_page',
